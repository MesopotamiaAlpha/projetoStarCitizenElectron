# Companheiro Emoto — Manual do Programador
...
## 1.1. Funcionalidades recentes e pontos de manutenção

| Área | Implementação atual | Arquivo principal |
|---|---|---|
| Desempenho de Minha Coleção | Atualização seletiva de quantidade: somente o set e a peça afetados são recriados; estatísticas estruturais não são recalculadas no clique. | `src/pages/MyCollectionPage.js`, `src/data/armorPerformance.js` |
| Acompanhamento UEX | Índices por nome normalizado para armaduras, sets e inventário; consultas de estoque deixam de percorrer toda a base para cada anúncio. | `src/pages/UexSalesPage.js` |
| UEX em lote | Sincronização permite adicionar todos os anúncios encontrados em uma única operação, mantendo deduplicação. | `src/pages/UexSalesPage.js`, `src/data/uexImportBatch.js` |
| Alertas de Compra | Foco originado pelo sininho é consumido uma única vez; não deve ser reintroduzido em polling ou em cada renderização. | `src/data/uexMarketAlerts.js`, `src/pages/MarketAlertsPage.js`, `src/pages/UexInsightsPage.js` |
| Wikelo Favor | Cada favor faltante equivale a 50 scrip; o saldo soma `MG Scrip`, `Council Scrip` e `ConCuI Scrip` no inventário. | `src/data/wikeloInventory.js`, `src/pages/WikeloTrackerPage.js` |
| Limpeza seletiva | Apaga módulos escolhidos após snapshot automático do SQLite. | `src/pages/DataDirectoryPage.js`, `electron/main.js` |
| Acessibilidade visual | Tokens de texto secundário e muted com contraste maior, tipografia auxiliar mínima, placeholders claros e foco visível. | `src/App.css` |
| Calculadora e chat | Listener global de teclado ignora inputs, textareas, selects e elementos editáveis. | `src/components/CalculatorWidget.js` |

Ao alterar qualquer uma dessas áreas, execute primeiro os testes direcionados e depois `npm run verify`. Para mudanças em eventos, confirme sempre que o evento é publicado uma vez e que listeners são removidos no cleanup do `useEffect`.
...
├── Manual Usuário — Companheiro Emoto.md  Manual de uso para usuário final
├── README.md                   Apresentação pública do projeto no GitHub
└── MANUAL-DO-PROGRAMADOR.md   Este manual técnico
```
...
O manual final para o usuário está em `Manual Usuário — Companheiro Emoto.md`. O README público está em `README.md`; este arquivo é o manual técnico do programador. Os guias de instalação e de limpeza seletiva em `docs/` são documentos operacionais complementares.
...
O comando `npm run verify` é o mínimo obrigatório. Na validação mais recente foram aprovadas **37 suítes e 153 testes**, o build React foi compilado e os arquivos `electron/main.js`, `electron/preload.js` e `electron/missionWatcher.js` passaram na checagem sintática. Sempre repita os comandos após modificar o processo principal, pois o build React não valida automaticamente os handlers IPC.
